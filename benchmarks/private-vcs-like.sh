...
# Allow the isolated local git:// fixture only inside this temporary Composer home.
...
export BENCH_GIT_LOG="$WORK/git-calls.log"
...
measure() { local s e; : > "$BENCH_GIT_LOG"; s="$(ms_now)"; "$@" >/dev/null 2>&1; e="$(ms_now)"; MEASURE_MS=$(( (e-s)/1000000 )); MEASURE_CALLS="$(wc -l < "$BENCH_GIT_LOG" | tr -d ' ')"; }
...
run_series() {
  ...
  for _ in $(seq 1 "$RUNS"); do
    reset_root
    measure "$@"
    times+=("$MEASURE_MS"); calls+=("$MEASURE_CALLS")
  done
  printf 'PRIVATE_BENCH|%s|%s|%s|calls=%s|%s\n' "$label" "$(median "${times[@]}")" "$(IFS=,; echo "${times[*]}")" "$(median "${calls[@]}")" "$(IFS=,; echo "${calls[*]}")"
}
...
# Prime Fast Composer once from the same warm Composer cache.
...
# Warm targeted no-op; restore the exact same caches before each implementation.
...
# Broad no-op while Fast Composer's ref snapshot is still fresh.
...
# Move the explicitly required dev-feature branch.
...
composer_dev=(); composer_dev_calls=(); fast_dev=(); fast_dev_calls=()
for _ in $(seq 1 "$RUNS"); do
  reset_root; restore_dir "$WORK/cache-baseline" "$COMPOSER_HOME"; measure composer update "$TARGET" "${FLAGS[@]}"; composer_dev+=("$MEASURE_MS"); composer_dev_calls+=("$MEASURE_CALLS")
  reset_root; restore_dir "$WORK/cache-baseline" "$COMPOSER_HOME"; restore_dir "$WORK/fast-baseline" "$FAST_COMPOSER_CACHE_DIR"; measure "${FC[@]}" update "$TARGET" "${FLAGS[@]}"; fast_dev+=("$MEASURE_MS"); fast_dev_calls+=("$MEASURE_CALLS")
done
printf 'PRIVATE_BENCH|composer moved dev branch|%s|%s|calls=%s|%s\n' "$(median "${composer_dev[@]}")" "$(IFS=,; echo "${composer_dev[*]}")" "$(median "${composer_dev_calls[@]}")" "$(IFS=,; echo "${composer_dev_calls[*]}")"
printf 'PRIVATE_BENCH|fast-composer moved dev branch|%s|%s|calls=%s|%s\n' "$(median "${fast_dev[@]}")" "$(IFS=,; echo "${fast_dev[*]}")" "$(median "${fast_dev_calls[@]}")" "$(IFS=,; echo "${fast_dev_calls[*]}")"
