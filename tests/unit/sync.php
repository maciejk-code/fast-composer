...
fc_assert(($state['packages']['acme/a']['1.0.0']['source']['reference'] ?? null) === $lockedSha, 'lock package snapshot failed');
fc_assert(
    isset($state['packages']['acme/a']['dev-unused-a'], $state['packages']['acme/a']['dev-unused-b'], $state['packages']['acme/a']['dev-main']),
    'sync did not index every branch: '.implode(', ', array_keys($state['packages']['acme/a'] ?? []))
);
...
fc_assert(
    $synced === 1 && ($state['repos'][$repo2]['name'] ?? null) === 'acme/b' && isset($state['packages']['acme/b']['dev-main']),
    'incremental sync did not add only the new repository (synced='.$synced.')'
);
$synced = (new Snapshot($root))->sync($state, $config);
fc_assert($synced === 0 && !isset($state['packages']['acme/b']) && isset($state['packages']['acme/a']), 'removing a repository did not drop exactly its packages');
...
fc_assert(
    glob($base.'/cache/mirrors/*.git') === $mirrorsBefore && isset($otherState['packages']['acme/a']['dev-unused-a']),
